...
function clearForm(form) {
  ...
  form.querySelectorAll("input:last-of-type").checked = false;
  const formClone = form.cloneNode(true);
  form.replaceWith(formClone);
  formClone.querySelector("button").addEventListener("click", exitForm);
  formClone.classList.add("hidden"); // Assumes that all use of this function requires it to be hidden afterwards
}
...
function editEntry(event) {
  event.preventDefault();
  const formData = new FormData(event.target);
  const entries = Array.from(this.querySelectorAll("p"));
...
Book.prototype.penSVG = {
  ...
  clickEvent() {
    ...
    openForm.bind(editEntry.bind(article)).call();
  },
};
